  ...
  set F : AlgebraNorm K L :=
    { RingSeminorm.toRingNorm F' hF'_0 with
      smul' := fun k y ↦ by
        ...
        simp only [hfk, hF']
        -- Unfold the `toRingNorm` wrapper so the multiplicativity lemma rewrites at reducible
        -- transparency.
        simp only [RingSeminorm.toRingNorm, RingSeminorm.toFun_eq_coe,
          AddGroupSeminorm.toFun_eq_coe]
        rw [← smoothingSeminorm_of_mul f hf_1 hf_na hk y, Algebra.smul_def] }
