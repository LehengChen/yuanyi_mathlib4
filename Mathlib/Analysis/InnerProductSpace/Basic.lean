...
theorem norm_inner_eq_norm_tfae (x y : E) :
  ...
  tfae_have 1 → 2 := by
    ...
    have hxy : InnerProductSpace.Core.normSq (𝕜 := 𝕜) (F := E)
        (⟪x, y⟫ • x - ⟪x, x⟫ • y) = 0 := by
      rw [InnerProductSpace.Core.cauchy_schwarz_aux (𝕜 := 𝕜) (F := E)]
      simpa [InnerProductSpace.Core.normSq] using h
    rw [InnerProductSpace.Core.normSq_eq_zero (𝕜 := 𝕜) (F := E), sub_eq_zero] at hxy
    rw [div_eq_inv_mul, mul_smul, hxy, inv_smul_smul₀]
