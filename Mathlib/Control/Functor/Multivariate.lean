...
private def g :
  ...
  | _, α, Fin2.fs i, x =>
    ⟨x.val, by
      simpa [PredLast', TypeVec.splitFun] using
        (const_iff_true (α := α) (i := i) (x := x.val) (p := True)).2 x.property⟩
  | _, _, Fin2.fz, x => ⟨x.val, x.property⟩
...
private def f' :
  ...
  | _, α, Fin2.fs i, x =>
    ⟨x.val, by
      simpa [RelLast] using
        (repeatEq_iff_eq (α := α) (i := i) (x := x.val.1) (y := x.val.2)).1 <| by
          simpa [RelLast', TypeVec.splitFun] using x.property⟩
  | _, _, Fin2.fz, x => ⟨x.val, x.property⟩
...
private def g' :
  ...
  | _, α, Fin2.fs i, x =>
    ⟨x.val, by
      simpa [RelLast', TypeVec.splitFun] using
        (repeatEq_iff_eq (α := α) (i := i) (x := x.val.1) (y := x.val.2)).2 x.property⟩
  | _, _, Fin2.fz, x => ⟨x.val, x.property⟩
