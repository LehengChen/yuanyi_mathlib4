...
def StarModule.decomposeProdAdjoint : A ≃ₗ[R] selfAdjoint A × skewAdjoint A := by
  ...
  -- Unfold these submodule wrappers explicitly so the subtype maps reduce to `Subtype.val`.
  ext x <;> dsimp [Submodule.subtype, selfAdjoint.submodule, skewAdjoint.submodule] <;> simp
