...
theorem map'_comp {M₁ M₂ M₃ : ModuleCat.{v} R} (l₁₂ : M₁ ⟶ M₂) (l₂₃ : M₂ ⟶ M₃) :
  ...
  | add x y ihx ihy =>
      calc
        ...
        _ = (Hom.hom (map' f l₁₂ ≫ map' f l₂₃)) x +
            (Hom.hom (map' f l₁₂ ≫ map' f l₂₃)) y := by
              rw [ihx, ihy]
        _ = (Hom.hom (map' f l₁₂ ≫ map' f l₂₃)) (x + y) := by
              exact ((Hom.hom (map' f l₁₂ ≫ map' f l₂₃)).map_add x y).symm
  ...
  { toFun := fun y : Y =>
      { toFun := fun s : S => g <| (s • y : Y)
        ...
        map_smul' := fun r (s : S) => by
          ...
          dsimp
          rw [← g.hom.map_smul]
          have hs : (r • s : S) = f r * s := by
            simpa [smul_eq_mul] using
              (ModuleCat.restrictScalars.smul_def' (f := f) (M := ModuleCat.of S S) r s)
          rw [hs, mul_smul]
          rfl }
    map_add' := fun y1 y2 : Y =>
...
def HomEquiv.toRestriction {X Y} (g : Y ⟶ (coextendScalars f).obj X) :
  ...
  { toFun := fun y : Y => (g y) (1 : S)
    ...
    map_smul' := fun r (y : Y) => by
      ...
      have h1 : (r • (1 : S) : S) = f r := by
        simpa [smul_eq_mul] using
          (ModuleCat.restrictScalars.smul_def' (f := f) (M := ModuleCat.of S S) r (1 : S))
      have h2 := (g y).map_smul r (1 : S)
      rw [h1] at h2
      exact h2 }
        ...
        map_smul' := fun r (s : S) => by
          ...
          have hs : (r • s : S) = f r * s := by
            simpa [smul_eq_mul] using
              (ModuleCat.restrictScalars.smul_def' (f := f) (M := ModuleCat.of S S) r s)
          rw [hs, mul_smul]
          exact (ModuleCat.restrictScalars.smul_def' (f := f) r (s • y : Y)).symm }
    map_add' := fun y1 y2 =>
      LinearMap.ext fun s : S => by
...
def HomEquiv.toRestrictScalars {X Y} (g : (extendScalars f).obj X ⟶ Y) :
  ...
  { toFun := fun x => g <| (1 : S) ⊗ₜ[R,f] x
    ...
    map_smul' := fun r s => by
      letI : Module R S := Module.compHom S f
      letI : Module R Y := Module.compHom Y f
      have hs := ModuleCat.restrictScalars.smul_def' (f := f) r
        (g.hom ((1 : S) ⊗ₜ[R] s) : Y)
      have h1 : (r • (1 : S) : S) = f r := by
        simpa [smul_eq_mul] using
          (ModuleCat.restrictScalars.smul_def' (f := f) (M := ModuleCat.of S S) r (1 : S))
      have ht' : (1 : S) ⊗ₜ[R] (r • s) = f r ⊗ₜ[R] s := by
        calc
          (1 : S) ⊗ₜ[R] (r • s) = (r • (1 : S)) ⊗ₜ[R] s := TensorProduct.tmul_smul (R := R) r (1 : S) s
          _ = f r ⊗ₜ[R] s := by rw [h1]
      calc
        g.hom ((1 : S) ⊗ₜ[R] (r • s)) = g.hom (f r ⊗ₜ[R] s) := by
          rw [ht']
        _ = g.hom (f r • ((1 : S) ⊗ₜ[R] s)) := by
          rw [ExtendScalars.smul_tmul, mul_one]
        _ = f r • g.hom ((1 : S) ⊗ₜ[R] s) := g.hom.map_smul _ _
        _ = r • (g.hom ((1 : S) ⊗ₜ[R] s) : Y) := hs.symm }
...
def homEquiv {X Y} :
  ...
  left_inv g := by
    ...
    | tmul x s =>
      change (show S from x) • (ConcreteCategory.hom g) ((1 : S) ⊗ₜ[R] s) =
        (ConcreteCategory.hom g) ((show S from x) ⊗ₜ[R] s)
      rw [← map_smul, ExtendScalars.smul_tmul, mul_one]
    | add _ _ ih1 ih2 => rw [map_add, map_add, ih1, ih2]
  right_inv g := by
    letI m1 : Module R S := Module.compHom S f; letI m2 : Module R Y := Module.compHom Y f
    ext x
    rw [HomEquiv.toRestrictScalars_hom_apply]
    simp [HomEquiv.fromExtendScalars, TensorProduct.lift.tmul]
...
def counit : restrictScalars.{max v u₂, u₁, u₂} f ⋙ extendScalars f ⟶ 𝟭 (ModuleCat S) where
  ...
  naturality Y Y' g := by
    ...
    | tmul s' y =>
      dsimp
      simp [Counit.map, TensorProduct.lift.tmul, map_smul]
  ...
  Adjunction.mk' {
    ...
    homEquiv_counit := fun {X Y g} ↦ hom_ext <| LinearMap.ext fun x => by
        ...
        | tmul =>
          ...
          dsimp
          simp [ExtendRestrictScalarsAdj.Counit.map,
            ExtendRestrictScalarsAdj.HomEquiv.fromExtendScalars]
...
lemma extendScalars_assoc :
  ...
  have h₂' := congrArg
    (fun z =>
      (Hom.hom ((extendScalarsComp f₂₃ f₃₄).hom.app ((extendScalars f₁₂).obj M))) z) h₂
  have h₃' := h₃ ((extendScalars f₁₂).obj M) ((1 : R₂) ⊗ₜ[R₁,f₁₂] m)
  dsimp at h₁ h₂ h₃ h₄ ⊢
  rw [h₁]
  rw [ExtendScalars.map_tmul, h₄]
...
lemma extendScalars_id_comp :
  ...
  dsimp
  have h := congrArg
    (fun z => (Hom.hom ((extendScalars f₁₂).map ((extendScalarsId R₁).hom.app M))) z)
    (extendScalarsComp_hom_app_one_tmul (RingHom.id R₁) f₁₂ M m)
  dsimp at h
  refine h.trans ?_
  rw [ExtendScalars.map_tmul]
  have h' := congrArg (fun z => (1 : R₂) ⊗ₜ[R₁,f₁₂] z) (extendScalarsId_hom_app_one_tmul M m)
  simpa using h'
...
lemma extendScalars_comp_id :
  ...
  dsimp
  have h := congrArg
    (fun z => (Hom.hom ((extendScalarsId R₂).hom.app ((extendScalars f₁₂).obj M))) z)
    (extendScalarsComp_hom_app_one_tmul f₁₂ (RingHom.id R₂) M m)
  dsimp at h
  refine h.trans ?_
  simpa using
    (extendScalarsId_hom_app_one_tmul ((extendScalars f₁₂).obj M) ((1 : R₂) ⊗ₜ[R₁,f₁₂] m))
