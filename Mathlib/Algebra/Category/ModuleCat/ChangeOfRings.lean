...
theorem map'_comp {M₁ M₂ M₃ : ModuleCat.{v} R} (l₁₂ : M₁ ⟶ M₂) (l₂₃ : M₂ ⟶ M₃) :
  ...
  | add x y ihx ihy =>
      calc
        (Hom.hom (map' f (l₁₂ ≫ l₂₃))) (x + y)
            = (Hom.hom (map' f (l₁₂ ≫ l₂₃))) x + (Hom.hom (map' f (l₁₂ ≫ l₂₃))) y := by
                exact (Hom.hom (map' f (l₁₂ ≫ l₂₃))).map_add x y
        _ = (Hom.hom (map' f l₂₃)) ((Hom.hom (map' f l₁₂)) x) +
              (Hom.hom (map' f l₂₃)) ((Hom.hom (map' f l₁₂)) y) := by
                simpa [ModuleCat.hom_comp] using congrArg₂ (fun a b => a + b) ihx ihy
        _ = (Hom.hom (map' f l₂₃)) ((Hom.hom (map' f l₁₂)) (x + y)) := by
              rw [← (Hom.hom (map' f l₂₃)).map_add, ← (Hom.hom (map' f l₁₂)).map_add]
              rfl
  ...
  { toFun := fun y : Y =>
      { toFun := fun s : S => g <| (s • y : Y)
        ...
        map_smul' := fun r (s : S) => by
          ...
          dsimp
          rw [← g.hom.map_smul]
          change (ConcreteCategory.hom g) ((f r * s) • y) = (Hom.hom g) (f r • s • y)
          rw [mul_smul] }
    map_add' := fun y1 y2 : Y =>
...
def HomEquiv.toRestrictScalars {X Y} (g : (extendScalars f).obj X ⟶ Y) :
  ...
  { toFun := fun x => g <| (1 : S) ⊗ₜ[R,f] x
    ...
    map_smul' := fun r s => by
      ...
      dsimp
      change (ConcreteCategory.hom g) ((1 : S) ⊗ₜ[R] (r • s)) =
        r • (ConcreteCategory.hom g) ((1 : S) ⊗ₜ[R] s)
      rw [tmul_smul]
      change (ConcreteCategory.hom g) ((f r) • ((1 : S) ⊗ₜ[R] s)) =
        f r • (ConcreteCategory.hom g) ((1 : S) ⊗ₜ[R] s)
      exact g.hom.map_smul (f r) ((1 : S) ⊗ₜ[R] s) }
...
def homEquiv {X Y} :
  ...
  left_inv g := by
    ...
    | tmul x s =>
      change (Hom.hom (HomEquiv.fromExtendScalars f (HomEquiv.toRestrictScalars f g)))
          ((show ↑((restrictScalars f).obj (of _ S)) from x) ⊗ₜ[R] s) =
        (Hom.hom g) (x ⊗ₜ[R] s)
  ...
  right_inv g := by
    ...
    -- Use a local alias so the domain matches after unfolding `fromExtendScalars`.
    let one' : ↑((restrictScalars f).obj (of _ S)) :=
      show ↑((restrictScalars f).obj (of _ S)) from (1 : S)
    change (Hom.hom (HomEquiv.fromExtendScalars f g)) (one' ⊗ₜ[R] x) = (Hom.hom g) x
    simp [HomEquiv.fromExtendScalars, HomEquiv.evalAt, one']
...
def counit : restrictScalars.{max v u₂, u₁, u₂} f ⋙ extendScalars f ⟶ 𝟭 (ModuleCat S) where
  ...
  naturality Y Y' g := by
    ...
    | tmul s' y =>
      dsimp
      -- Align the scalar locally before changing the goal shape.
      let s'' : S := s'
      change (Hom.hom (Counit.map f)) (s'' ⊗ₜ[R] (ConcreteCategory.hom g) y) =
        (Hom.hom g) ((Hom.hom (Counit.map f)) (s' ⊗ₜ[R] y))
      simpa [Counit.map] using
        (Counit.map_hom_apply (f := f) (a := s'' ⊗ₜ[R] (ConcreteCategory.hom g) y))
  ...
  Adjunction.mk' {
    ...
    homEquiv_counit := fun {X Y g} ↦ hom_ext <| LinearMap.ext fun x => by
        ...
        | tmul x y =>
          ...
          -- Align the scalar locally before changing the goal shape.
          let x' : S := x
          change (Hom.hom (ExtendRestrictScalarsAdj.HomEquiv.fromExtendScalars f g))
              ((show ↑((restrictScalars f).obj (of _ S)) from x) ⊗ₜ[R] y) =
            x' • (ConcreteCategory.hom g) y
          simp [x', ExtendRestrictScalarsAdj.HomEquiv.fromExtendScalars,
            ExtendRestrictScalarsAdj.HomEquiv.evalAt]
...
lemma extendScalars_assoc :
  ...
  dsimp at h₁ h₂ h₃ h₄ ⊢
  have h₁' := congrArg
    (Hom.hom ((extendScalars f₃₄).map ((extendScalarsComp f₁₂ f₂₃).hom.app M))) h₁
  have h₂' := congrArg
    (Hom.hom ((extendScalarsComp f₂₃ f₃₄).hom.app ((extendScalars f₁₂).obj M))) h₂
  exact h₁'.trans <| by
    rw [ExtendScalars.map_tmul]
    simpa [h₄] using
      (h₃ ((extendScalars f₁₂).obj M) ((1 : R₂) ⊗ₜ[R₁] m)).symm.trans h₂'.symm
...
lemma extendScalars_id_comp :
  ...
  dsimp
  have h := congrArg
    (Hom.hom ((extendScalars f₁₂).map ((extendScalarsId R₁).hom.app M)))
    (extendScalarsComp_hom_app_one_tmul (RingHom.id R₁) f₁₂ M m)
  exact h.trans (by
    rw [ExtendScalars.map_tmul, extendScalarsId_hom_app_one_tmul]
    rfl)
...
lemma extendScalars_comp_id :
  ...
  have h := congrArg
    ...
    (extendScalarsComp_hom_app_one_tmul f₁₂ (RingHom.id R₂) M m)
  exact h.trans (by
    simpa using extendScalarsId_hom_app_one_tmul ((extendScalars f₁₂).obj M)
      ((1 : R₂) ⊗ₜ[R₁] m))
