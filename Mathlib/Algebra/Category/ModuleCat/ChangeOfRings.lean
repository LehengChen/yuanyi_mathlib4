...
@[simp] theorem map'_hom {M1 M2 : ModuleCat.{v} R} (l : M1 ⟶ M2) :
    (map' f l).hom =
      @LinearMap.baseChange R S M1 M2 _ _ ((algebraMap S _).comp f).toAlgebra _ _ _ _ l.hom := rfl
...
theorem map'_comp {M₁ M₂ M₃ : ModuleCat.{v} R} (l₁₂ : M₁ ⟶ M₂) (l₂₃ : M₂ ⟶ M₃) :
  ...
  ext x
  simp [ModuleCat.hom_comp, LinearMap.baseChange_comp]
  rfl
...
@[simp] lemma restrictScalars_self_smul (r : R) (s : (restrictScalars f).obj (of S S)) :
    r • s = (f r * (show S from s) : S) := rfl
...
def HomEquiv.toRestriction {X Y} (g : Y ⟶ (coextendScalars f).obj X) :
  ...
  { toFun := fun y : Y => (g y) (1 : S)
    ...
    map_smul' := fun r (y : Y) => by
      dsimp
      rw [← map_smul]
      simp [map_smul, CoextendScalars.smul_apply] }
...
def HomEquiv.toRestrictScalars {X Y} (g : (extendScalars f).obj X ⟶ Y) :
  ...
  { toFun := fun x => g <| (1 : S) ⊗ₜ[R,f] x
    ...
    map_smul' := fun r s => by
      ...
      dsimp
      rw [← LinearMap.map_smul, tmul_smul]
      congr }
...
def HomEquiv.fromExtendScalars {X Y} (g : X ⟶ (restrictScalars f).obj Y) :
  ...
  · intro s z
    ...
    | tmul s' x =>
      ...
      rw [lift.tmul', lift.tmul']
      set s' : S := s'
      change (s * s') • (g x) = s • s' • (g x)
      rw [mul_smul]
    | add _ _ ih1 ih2 => rw [smul_add, map_add, ih1, ih2, map_add, smul_add]

@[simp] lemma HomEquiv.fromExtendScalars_hom_tmul {X Y} (g : X ⟶ (restrictScalars f).obj Y)
    (s : S) (x : X) : (HomEquiv.fromExtendScalars f g).hom (s ⊗ₜ[R,f] x) = s • (g x : Y) :=
  rfl
...
def homEquiv {X Y} :
  ...
  left_inv g := by
    ...
    | tmul x s =>
      simp only [HomEquiv.fromExtendScalars_hom_tmul, HomEquiv.toRestrictScalars_hom_apply]
      change S at x
      rw [← g.hom.map_smul x ((1 : S) ⊗ₜ[R,f] s), ExtendScalars.smul_tmul, mul_one x]
    | add _ _ ih1 ih2 => rw [map_add, map_add, ih1, ih2]
  right_inv g := by
    letI m1 : Module R S := Module.compHom S f; letI m2 : Module R Y := Module.compHom Y f
    ext x
    rw [HomEquiv.toRestrictScalars_hom_apply]
    simp only [HomEquiv.fromExtendScalars_hom_tmul, one_smul]
