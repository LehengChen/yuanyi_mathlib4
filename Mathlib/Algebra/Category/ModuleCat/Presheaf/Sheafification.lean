  ...
  Adjunction.mkOfHomEquiv
    { homEquiv := fun _ _ ↦ sheafificationHomEquiv α
        ...
        calc
          (SheafOfModules.toSheaf R).map ((sheafificationHomEquiv α).symm (f ≫ g)) =
              (((CategoryTheory.sheafificationAdjunction J AddCommGrpCat).homEquiv P₀.presheaf
                ((SheafOfModules.toSheaf R).obj N)).symm ((toPresheaf R₀).map (f ≫ g))) := by
                  simpa using
                    (toSheaf_map_sheafificationHomEquiv_symm (α := α) (g := f ≫ g))
          _ = (SheafOfModules.toSheaf R).map ((sheafification α).map f) ≫
                (((CategoryTheory.sheafificationAdjunction J AddCommGrpCat).homEquiv Q₀.presheaf
                  ((SheafOfModules.toSheaf R).obj N)).symm ((toPresheaf R₀).map g)) := by
                    rw [Functor.map_comp]
                    exact (CategoryTheory.sheafificationAdjunction J
                      AddCommGrpCat.{v}).homEquiv_naturality_left_symm ((toPresheaf R₀).map f)
                        (show Q₀.presheaf ⟶
                            (sheafToPresheaf J AddCommGrpCat).obj ((SheafOfModules.toSheaf R).obj N)
                          from (toPresheaf R₀).map g)
          _ = (SheafOfModules.toSheaf R).map ((sheafification α).map f) ≫
      ...
      homEquiv_naturality_right := fun {P₀ M N} f g ↦ by
        apply (toPresheaf _).map_injective
        rw [Functor.map_comp]
        simpa [toPresheaf_map_sheafificationHomEquiv, Functor.map_comp] using
          (CategoryTheory.sheafificationAdjunction J
            AddCommGrpCat.{v}).homEquiv_naturality_right
