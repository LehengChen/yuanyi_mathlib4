...
theorem isThin_iff (C : Type*) [Category C] [IsGroupoid C] :
    Quiver.IsThin C ↔ ∀ c : C, Subsingleton (c ⟶ c) := by
  refine ⟨fun h c => h c c, fun h c d => Subsingleton.intro fun f g => ?_⟩
  haveI := h d
  calc
    f = f ≫ CategoryTheory.inv g ≫ g := by
      simp only [IsIso.inv_hom_id, Category.comp_id]
    _ = f ≫ CategoryTheory.inv f ≫ g := by
      congr 1
      simp only [IsIso.inv_hom_id, eq_iff_true_of_subsingleton]
    _ = g := by simp only [IsIso.hom_inv_id_assoc]
