  ...
  map f :=
    { hom₁ := f.hom₁⟦n⟧'
      ...
      comm₃ := by
        ...
        simp only [← Functor.comp_map, (shiftFunctorComm C 1 n).hom.naturality] }
  ...
  NatIso.ofComponents
    (fun T => Triangle.isoMk _ _
      ...
      (by
        ...
        simp only [← Functor.comp_map, ← NatTrans.naturality_assoc,
          shiftFunctorAdd'_eq_shiftFunctorAdd, Int.negOnePow_add,
