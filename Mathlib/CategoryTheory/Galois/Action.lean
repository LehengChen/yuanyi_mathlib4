...
instance (X : C) [GaloisCategory C] [FiberFunctor F] [IsGalois X] :
    MulAction.IsPretransitive (Aut X) ((functorToAction F).obj X).V :=
  isPretransitive_of_isGalois F X

instance [Functor.Faithful F] : Functor.Faithful (functorToAction F) :=
...
instance [PreservesMonomorphisms F] : PreservesMonomorphisms (functorToAction F) :=
...
instance [Functor.Faithful F] : ReflectsMonomorphisms (functorToAction F) :=
  reflectsMonomorphisms_of_faithful _

instance [F.ReflectsIsomorphisms] : Functor.ReflectsIsomorphisms (functorToAction F) where
  reflects f _ :=
    have : IsIso (F.map f) := (forget₂ _ FintypeCat).map_isIso ((functorToAction F).map f)
    isIso_of_reflects_iso f F

noncomputable instance [PreservesFiniteCoproducts F] :
    PreservesFiniteCoproducts (functorToAction F) :=
...
noncomputable instance [PreservesFiniteProducts F] :
    PreservesFiniteProducts (functorToAction F) :=
...
instance [GaloisCategory C] [FiberFunctor F] : PreservesIsConnected (functorToAction F) :=
