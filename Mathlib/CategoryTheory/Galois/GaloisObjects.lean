...
* `isGalois_iff_pretransitive` : A connected object `X` is Galois if and only if `Aut X`
...
/-- For Galois `A` and a point `a` of the fiber of `A`, evaluation at `a` as an equivalence. -/
