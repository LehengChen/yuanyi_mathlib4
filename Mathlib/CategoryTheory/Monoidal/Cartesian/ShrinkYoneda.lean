...
lemma shrinkYonedaMon_obj_map_shrinkYonedaMonObjObjEquiv_symm
    {M : C} [MonObj M] {Y Y' : Cᵒᵖ} (g : Y ⟶ Y') (f : Y.unop ⟶ M) :
    (shrinkYonedaMon.{w}.obj (Mon.mk M)).map g
        ((shrinkYonedaMonObjObjEquiv (M := Mon.mk M)).symm f) =
      (shrinkYonedaMonObjObjEquiv (M := Mon.mk M)).symm (g.unop ≫ f) := by
  simp [shrinkYonedaMon, shrinkYonedaMonObjObjEquiv]

lemma shrinkYonedaMonObjObjEquiv_symm_comp {M : C} [MonObj M] {Y Y' : C} (g : Y' ⟶ Y)
    (f : Y ⟶ M) :
    (shrinkYonedaMonObjObjEquiv (M := Mon.mk M)).symm (g ≫ f) =
    (shrinkYonedaMon.obj (Mon.mk M)).map g.op
      ((shrinkYonedaMonObjObjEquiv (M := Mon.mk M)).symm f) :=
...
lemma shrinkYonedaMon_map_app_shrinkYonedaObjObjEquiv_symm
    {M M' : C} [MonObj M] [MonObj M'] {Y : Cᵒᵖ} (f : Y.unop ⟶ M) (g : M ⟶ M')
    [IsMonHom g] :
    (shrinkYonedaMon.map (Mon.Hom.mk g)).app _
        ((shrinkYonedaMonObjObjEquiv (M := Mon.mk M)).symm f) =
      (shrinkYonedaMonObjObjEquiv (M := Mon.mk M')).symm (f ≫ g) := by
  simp [shrinkYonedaMon, shrinkYonedaMonObjObjEquiv]
...
lemma shrinkYonedaGrp_obj_map_shrinkYonedaGrpObjObjEquiv_symm
    {M : C} [GrpObj M] {Y Y' : Cᵒᵖ} (g : Y ⟶ Y') (f : Y.unop ⟶ M) :
    (shrinkYonedaGrp.{w}.obj (Grp.mk M)).map g
        ((shrinkYonedaGrpObjObjEquiv (M := Grp.mk M)).symm f) =
      (shrinkYonedaGrpObjObjEquiv (M := Grp.mk M)).symm (g.unop ≫ f) := by
  simp [shrinkYonedaGrp, shrinkYonedaGrpObjObjEquiv]

lemma shrinkYonedaGrpObjObjEquiv_symm_comp {M : C} [GrpObj M] {Y Y' : C} (g : Y' ⟶ Y)
    (f : Y ⟶ M) :
    (shrinkYonedaGrpObjObjEquiv (M := Grp.mk M)).symm (g ≫ f) =
    (shrinkYonedaGrp.obj (Grp.mk M)).map g.op
      ((shrinkYonedaGrpObjObjEquiv (M := Grp.mk M)).symm f) :=
...
lemma shrinkYonedaGrp_map_app_shrinkYonedaObjObjEquiv_symm
    {M M' : C} [GrpObj M] [GrpObj M'] {Y : Cᵒᵖ} (f : Y.unop ⟶ M) (g : M ⟶ M')
    [IsMonHom g] :
    (shrinkYonedaGrp.map (Grp.homMk g)).app _
        ((shrinkYonedaGrpObjObjEquiv (M := Grp.mk M)).symm f) =
      (shrinkYonedaGrpObjObjEquiv (M := Grp.mk M')).symm (f ≫ g) := by
  simp [shrinkYonedaGrp, shrinkYonedaGrpObjObjEquiv]
