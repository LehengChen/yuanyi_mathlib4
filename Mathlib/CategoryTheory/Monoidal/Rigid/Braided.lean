...
private theorem coevaluation_evaluation_braided'
    {coevaluation : 𝟙_ C ⟶ X ⊗ Y} {evaluation : Y ⊗ X ⟶ 𝟙_ C}
    (h : coevaluation ▷ X ≫ (α_ X Y X).hom ≫ X ◁ evaluation =
      (λ_ X).hom ≫ (ρ_ X).inv) :
    X ◁ (coevaluation ≫ (β_ Y X).inv) ≫ (α_ X Y X).inv ≫
        ((β_ X Y).hom ≫ evaluation) ▷ X
  ...
  calc
    _ = 𝟙 X ⊗≫ X ◁ coevaluation ⊗≫ (X ◁ (β_ Y X).inv ⊗≫
        (β_ X Y).hom ▷ X) ⊗≫ evaluation ▷ X ⊗≫ 𝟙 X := by
      monoidal
    _ = 𝟙 X ⊗≫ X ◁ coevaluation ⊗≫ (𝟙 (X ⊗ X ⊗ Y) ⊗≫ (β_ X X).hom ▷ Y ⊗≫ X ◁ (β_ X Y).hom
          ⊗≫ (β_ Y X).inv ▷ X ⊗≫ Y ◁ (β_ X X).inv ⊗≫ 𝟙 ((Y ⊗ X) ⊗ X)) ⊗≫ evaluation ▷ X ⊗≫ 𝟙 X := by
      congr 3
    ...
    _ = 𝟙 X ⊗≫ (X ◁ coevaluation ≫ (β_ X (X ⊗ Y)).hom) ⊗≫
        ((β_ (Y ⊗ X) X).inv ≫ evaluation ▷ X) ⊗≫ 𝟙 X := by
    ...
    _ = _ := by
      ...
      suffices (λ_ X).inv ≫ coevaluation ▷ X ≫ (α_ X Y X).hom ≫
          X ◁ evaluation ≫ (ρ_ X).hom = 𝟙 X by
        simpa [monoidalComp] using this
      rw [reassoc_of% h]
      simp

set_option backward.privateInPublic true in
/-- evaluation_coevaluation' field of `ExactPairing Y X` in a braided category -/
private theorem evaluation_coevaluation_braided'
    {coevaluation : 𝟙_ C ⟶ X ⊗ Y} {evaluation : Y ⊗ X ⟶ 𝟙_ C}
    (h : Y ◁ coevaluation ≫ (α_ Y X Y).inv ≫ evaluation ▷ Y =
      (ρ_ Y).hom ≫ (λ_ Y).inv) :
    (coevaluation ≫ (β_ Y X).inv) ▷ Y ≫ (α_ Y X Y).hom ≫
        Y ◁ ((β_ X Y).hom ≫ evaluation) =
      (λ_ Y).hom ≫ (ρ_ Y).inv := by
  rw [Iso.eq_comp_inv, ← Iso.inv_comp_eq_id]
  calc
    _ = 𝟙 Y ⊗≫ coevaluation ▷ Y ⊗≫ ((β_ Y X).inv ▷ Y ⊗≫
        Y ◁ (β_ X Y).hom) ≫ Y ◁ evaluation ⊗≫ 𝟙 Y := by
      monoidal
    _ = 𝟙 Y ⊗≫ coevaluation ▷ Y ⊗≫ (𝟙 ((X ⊗ Y) ⊗ Y) ⊗≫ X ◁ (β_ Y Y).hom ⊗≫ (β_ X Y).hom ▷ Y
        ⊗≫ Y ◁ (β_ Y X).inv ⊗≫ (β_ Y Y).inv ▷ X ⊗≫ 𝟙 (Y ⊗ Y ⊗ X)) ⊗≫ Y ◁ evaluation ⊗≫ 𝟙 Y := by
      congr 3
    ...
    _ = 𝟙 Y ⊗≫ (coevaluation ▷ Y ≫ (β_ (X ⊗ Y) Y).hom) ⊗≫
        ((β_ Y (Y ⊗ X)).inv ≫ Y ◁ evaluation) ⊗≫ 𝟙 Y := by
    ...
    _ = _ := by
      ...
      suffices (ρ_ Y).inv ≫ Y ◁ coevaluation ≫ (α_ Y X Y).inv ≫
          evaluation ▷ Y ≫ (λ_ Y).hom = 𝟙 Y by
        simpa [monoidalComp] using this
      rw [reassoc_of% h]
      simp
