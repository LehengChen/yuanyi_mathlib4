...
theorem binaryCofan_isColimit_iff {X Y : Type u} (c : BinaryCofan X Y) :
  ...
  classical
    ...
    · rintro ⟨h⟩
      ...
      have h_image_compl :
          ((h.coconePointUniqueUpToIso (binaryCoproductColimit X Y)).inv '' Set.range Sum.inr)ᶜ =
            (h.coconePointUniqueUpToIso (binaryCoproductColimit X Y)).symm.toEquiv ''
              (Set.range Sum.inr)ᶜ := by
        simpa using
          (Set.image_compl_eq
            (h.coconePointUniqueUpToIso (binaryCoproductColimit X Y)).symm.toEquiv.bijective
            (f := (h.coconePointUniqueUpToIso (binaryCoproductColimit X Y)).symm.toEquiv)
            (s := Set.range Sum.inr)).symm
      rw [h_image_compl]
      simp
    · rintro ⟨h₁, h₂, h₃⟩
