...
instance pullback.map_isIso {W X Y Z S T : C} (f₁ : W ⟶ S) (f₂ : X ⟶ S) [HasPullback f₁ f₂]
    ...
    (eq₁ : f₁ ≫ i₃ = i₁ ≫ g₁) (eq₂ : f₂ ≫ i₃ = i₂ ≫ g₂) [IsIso i₁] [IsIso i₂] [Mono i₃] :
    IsIso (pullback.map f₁ f₂ g₁ g₂ i₁ i₂ i₃ eq₁ eq₂) := by
  refine ⟨⟨pullback.lift (pullback.fst g₁ g₂ ≫ inv i₁) (pullback.snd g₁ g₂ ≫ inv i₂) ?_, ?_, ?_⟩⟩
  · rw [← cancel_mono i₃]
    simp only [Category.assoc, eq₁, eq₂, IsIso.inv_hom_id_assoc, pullback.condition]
  · ext <;> simp
  · ext <;> simp
...
instance pushout.map_isIso {W X Y Z S T : C} (f₁ : S ⟶ W) (f₂ : S ⟶ X) [HasPushout f₁ f₂]
    ...
    (eq₁ : f₁ ≫ i₁ = i₃ ≫ g₁) (eq₂ : f₂ ≫ i₂ = i₃ ≫ g₂) [IsIso i₁] [IsIso i₂] [Epi i₃] :
    IsIso (pushout.map f₁ f₂ g₁ g₂ i₁ i₂ i₃ eq₁ eq₂) := by
  refine ⟨⟨pushout.desc (inv i₁ ≫ pushout.inl f₁ f₂) (inv i₂ ≫ pushout.inr f₁ f₂) ?_, ?_, ?_⟩⟩
  · rw [← cancel_epi i₃]
    rw [← Category.assoc i₃ g₁, ← Category.assoc i₃ g₂]
    rw [← eq₁, ← eq₂]
    rw [Category.assoc f₁ i₁, Category.assoc f₂ i₂]
    simp only [IsIso.hom_inv_id_assoc, pushout.condition]
  · ext <;> simp
  · ext <;> simp
