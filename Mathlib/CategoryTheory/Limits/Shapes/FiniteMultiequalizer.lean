...
variable [Fintype J.L] [Fintype J.R] in
instance : Fintype (WalkingMulticospan J) := .ofEquiv _ (proxy_equiv% (WalkingMulticospan J))

noncomputable instance instFinCategory [Finite J.L] [Finite J.R] :
    FinCategory (WalkingMulticospan J) := by
  classical
  letI : Fintype J.L := Fintype.ofFinite J.L
  letI : Fintype J.R := Fintype.ofFinite J.R
  exact
    { fintypeObj := .ofEquiv _ (proxy_equiv% (WalkingMulticospan J))
      fintypeHom
        | .left a, .left b => ⟨if e : a = b then {eqToHom (e ▸ rfl)} else ∅, by
            rintro ⟨⟩
            simp⟩
        ...
        | .right a, .right b => ⟨if e : a = b then {eqToHom (e ▸ rfl)} else ∅, by
            rintro ⟨⟩
            simp⟩ }
...
variable {J : MultispanShape}

variable [Fintype J.L] [Fintype J.R] in
instance : Fintype (WalkingMultispan J) := .ofEquiv _ (proxy_equiv% (WalkingMultispan J))

noncomputable instance instFinCategory [Finite J.L] [Finite J.R] :
    FinCategory (WalkingMultispan J) := by
  classical
  letI : Fintype J.L := Fintype.ofFinite J.L
  letI : Fintype J.R := Fintype.ofFinite J.R
  exact
    { fintypeObj := .ofEquiv _ (proxy_equiv% (WalkingMultispan J))
      fintypeHom
        | .left a, .left b => ⟨if e : a = b then {eqToHom (e ▸ rfl)} else ∅, by
            rintro ⟨⟩
            simp⟩
