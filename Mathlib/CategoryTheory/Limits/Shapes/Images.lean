  ...
  HasImageMap.mk
    { map := image.lift ((Image.monoFactorisation g.hom).ofArrowIso (inv sq))
      map_ι := by
        ...
        simpa only [Category.assoc, MonoFactorisation.ofArrowIso_m, ← Comma.comp_right,
          IsIso.hom_inv_id, Comma.id_right, Category.comp_id] using
          image.lift_fac ((Image.monoFactorisation g.hom).ofArrowIso (inv sq)) }
