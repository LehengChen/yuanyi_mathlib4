...
instance Pi.map_isIso {f g : β → C} [HasProduct f] [HasProduct g] (p : ∀ b, f b ⟶ g b)
    [∀ b, IsIso <| p b] : IsIso <| Pi.map p :=
  let α : Discrete.functor f ⟶ Discrete.functor g := Discrete.natTrans fun X ↦ p X.as
  haveI : ∀ X : Discrete β, IsIso (α.app X) := by
    intro X
    dsimp [α]
    infer_instance
  haveI : IsIso α := NatIso.isIso_of_isIso_app α
  inferInstanceAs (IsIso (limMap α))
...
instance Sigma.map_isIso {f g : β → C} [HasCoproduct f] [HasCoproduct g] (p : ∀ b, f b ⟶ g b)
  ...
  haveI : ∀ X : Discrete β, IsIso (α.app X) := by
    ...
    infer_instance
  haveI : IsIso α := NatIso.isIso_of_isIso_app α
  inferInstanceAs (IsIso (colimMap α))
