...
theorem jointly_surjective (k : K) [PreservesColimit F ((evaluation K (Type w)).obj k)]
    {t : Cocone F} (h : IsColimit t) (x : t.pt.obj k) :
...
theorem jointly_surjective' [HasColimit F] (k : K)
    [PreservesColimit F ((evaluation K (Type w)).obj k)] (x : (colimit F).obj k) :
    ∃ j y, x = (colimit.ι F j).app k y :=
  jointly_surjective F k (colimit.isColimit F) x
