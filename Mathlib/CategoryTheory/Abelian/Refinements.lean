...
lemma epi_iff_surjective_up_to_refinements [HasPullbacks C]
    [MorphismProperty.IsStableUnderBaseChange (MorphismProperty.epimorphisms C)] (f : X ⟶ Y) :
    Epi f ↔ ∀ ⦃A : C⦄ (y : A ⟶ Y),
      ∃ (A' : C) (π : A' ⟶ A) (_ : Epi π) (x : A' ⟶ X), π ≫ y = x ≫ f := by
  constructor
  · intro _ A a
    haveI : Epi (pullback.fst a f) :=
      (MorphismProperty.epimorphisms C).pullback_fst a f inferInstance
...
lemma surjective_up_to_refinements_of_epi [HasPullbacks C]
    [MorphismProperty.IsStableUnderBaseChange (MorphismProperty.epimorphisms C)]
    (f : X ⟶ Y) [Epi f] {A : C} (y : A ⟶ Y) :
    ∃ (A' : C) (π : A' ⟶ A) (_ : Epi π) (x : A' ⟶ X), π ≫ y = x ≫ f :=
  (epi_iff_surjective_up_to_refinements f).1 inferInstance y

variable [Preadditive C] [HasPullbacks C]
  [MorphismProperty.IsStableUnderBaseChange (MorphismProperty.epimorphisms C)]
  (S : ShortComplex C) {S₁ S₂ : ShortComplex C}

lemma ShortComplex.exact_iff_exact_up_to_refinements [S.HasHomology] :
...
lemma ShortComplex.eq_liftCycles_homologyπ_up_to_refinements [S.HasHomology]
    {A : C} (γ : A ⟶ S.homology) :
...
variable {K L} in
lemma ShortComplex.mono_homologyMap_iff_up_to_refinements
    (φ : S₁ ⟶ S₂) :
    Mono (homologyMap φ) ↔
...
variable {K L} in
lemma ShortComplex.epi_homologyMap_iff_up_to_refinements
    (φ : S₁ ⟶ S₂) :
    Epi (homologyMap φ) ↔
