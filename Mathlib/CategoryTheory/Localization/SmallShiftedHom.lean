...
lemma equiv_shift {a : M} [HasSmallLocalizedShiftedHom.{w} W M X Y]
  ...
  dsimp [equiv]
  have h' := congrArg (fun k => k ≫ (L.commShiftIso a').hom.app Y)
    (equiv_shift' (W := W) (L := L) f n a' h)
  simpa only [Functor.comp_obj, assoc, Iso.inv_hom_id_app, comp_id, Functor.map_comp] using h'
...
lemma equiv_comp [HasSmallLocalizedShiftedHom.{w} W M X Y]
  ...
  have h' := congrArg (fun k => k ≫ (L.commShiftIso c).hom.app Z)
    (SmallHom.equiv_comp (W := W) (L := L) f (g.shift a c h))
  simpa only [equiv_shift', Functor.comp_obj, assoc, Iso.inv_hom_id_app,
    comp_id, Functor.map_comp] using h'
