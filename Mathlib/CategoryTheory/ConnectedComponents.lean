...
instance (j : ConnectedComponents J) : IsConnected j.Component := by
  ...
  rintro ⟨j₁, hj₁⟩ ⟨j₂, rfl⟩
  induction Quotient.exact' hj₁ using Relation.ReflTransGen.head_induction_on with
  | refl => rfl
  | @head a b h h' ih =>
      have hab : Zag ({ obj := a, property := hj₁ } : (ConnectedComponents.mk j₂).Component)
          ({ obj := b, property := Quotient.sound' h' } : (ConnectedComponents.mk j₂).Component) :=
        zag_of_zag_obj (ConnectedComponents.ι _) h
      exact (Zigzag.of_zag hab).trans (ih (Quotient.sound' h'))
