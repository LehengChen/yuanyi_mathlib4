...
instance [ObjectProperty.EssentiallySmall.{w} P] [LocallySmall.{w} C] :
  ...
  exists_small_le' := by
    ...
    let R (a : α) : Prop := ∃ (Y : C) (h : Retract Y a.1.1), h.r ≫ h.i = a.2.1
    choose Y h hp using fun (a : Subtype R) ↦ a.2
    refine ⟨.ofObj Y, inferInstance, (monotone_retractClosure h₂).trans ?_⟩
    rw [retractClosure_isoClosure]
    rintro y ⟨x, hx, ⟨r⟩⟩
    let a : Subtype R := ⟨⟨⟨x, hx⟩, ⟨r.r ≫ r.i, by simp⟩⟩, ⟨y, r, rfl⟩⟩
    exact ⟨_, ⟨a.1, a.2⟩, ⟨{
      hom := r.i ≫ (h a).r
      inv := (h a).i ≫ r.r
      hom_inv_id := by simp [a, reassoc_of% (hp a)]
      inv_hom_id := by simp [a, ← reassoc_of% (hp a)]
    }⟩⟩
