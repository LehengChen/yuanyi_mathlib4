...
lemma IsPushout.hasLiftingProperty (h : IsPushout s f g t)
    {Z' W' : C} (g' : Z' ⟶ W')
    (hfg' : ∀ (u : Z ⟶ Z'), HasLiftingPropertyFixedTop f g' (s ≫ u) := by
      intro _ _ _
      infer_instance) :
  ...
  sq_hasLift := fun {u v} sq ↦ by
    ...
    letI : (CommSq.mk w).HasLift := hfg' u (t ≫ v) (CommSq.mk w)
...
lemma IsPullback.hasLiftingProperty (h : IsPullback s f g t)
    {X' Y' : C} (f' : X' ⟶ Y')
    (hfg : ∀ (v : Y' ⟶ Y), HasLiftingPropertyFixedBot f' g (v ≫ t) := by
      intro _ _ _
      infer_instance) :
  ...
  sq_hasLift := fun {u v} sq ↦ by
    ...
    letI : (CommSq.mk w).HasLift := hfg v (u ≫ s) (CommSq.mk w)
