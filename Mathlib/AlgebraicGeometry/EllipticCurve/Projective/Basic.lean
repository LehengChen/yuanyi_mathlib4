...
`Mathlib/AlgebraicGeometry/EllipticCurve/Projective` therefore often needs explicit rewrites to
move between these two forms. This is fairly self-contained here and does not cause any issues. It
would alternatively be possible to add some automation to assist here.
