...
lemma exists_basis_of_basis_baseChange [Module.FinitePresentation R M]
  ...
  let iequiv : (ι →₀ R) ≃ₗ[R] M := by
    ...
    · apply hi'.injective
      ...
      have hzero : LinearMap.lTensor k (i.comp i.ker.subtype) = 0 := by
        rw [(LinearMap.exact_subtype_ker_map i).linearMap_comp_eq_zero, LinearMap.lTensor_zero]
      have hxzero : (LinearMap.lTensor k (i.comp i.ker.subtype)) x = 0 := by
        exact congrArg (fun f => f x) hzero
      simpa [LinearMap.lTensor_comp, LinearMap.comp_apply] using hxzero
