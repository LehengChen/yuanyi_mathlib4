...
theorem liftR_map_last [lawful : LawfulMvFunctor F]
  ...
  have hh :
    ...
    · dsimp
      ...
      conv in (toSubtype fun i ↦ α.RelLast' R i.fs) => change toSubtype α.repeatEq
      have hsub :
          subtypeVal α.repeatEq ⊚ toSubtype α.repeatEq ⊚ dropFun c ⊚ diagSub =
            subtypeVal α.repeatEq ⊚ diagSub := by
        exact Eq.trans (by rfl)
          (congrArg (fun x => subtypeVal α.repeatEq ⊚ x)
            (toSubtype_of_subtype_assoc (p := α.repeatEq) (f := diagSub)))
      rw [hsub, diag_sub_val]
