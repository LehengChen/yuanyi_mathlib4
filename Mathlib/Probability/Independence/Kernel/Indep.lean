...
theorem IndepSets.indep_aux {m₂ m : MeasurableSpace Ω}
    {κ : Kernel α Ω} {μ : Measure α} {p1 p2 : Set (Set Ω)}
    (hκ : ∀ᵐ a ∂μ, IsZeroOrProbabilityMeasure (κ a)) (h2 : m₂ ≤ m) (hp2 : IsPiSystem p2)
    (hpm2 : m₂ = generateFrom p2) (hyp : IndepSets p1 p2 κ μ) {t1 t2 : Set Ω} (ht1 : t1 ∈ p1)
  ...
  | compl u hu ihu =>
    filter_upwards [ihu, hκ] with a ha hκa
    letI := hκa
    rcases eq_zero_or_isProbabilityMeasure (κ a) with hk | hk
    · simp [hk]
    · letI := hk
      rw [← Set.diff_eq, ← Set.diff_self_inter,
        measure_diff inter_subset_left (ht1m.inter (h2 _ hu)).nullMeasurableSet
          (measure_ne_top _ _),
...
/-- The measurable space structures generated by independent pi-systems are independent if
`κ a` is a zero-or-probability measure for `μ`-almost every `a`. -/
theorem IndepSets.indep_of_ae_isZeroOrProbabilityMeasure
    {m1 m2 m : MeasurableSpace Ω} {κ : Kernel α Ω} {μ : Measure α} {p1 p2 : Set (Set Ω)}
    (hκ : ∀ᵐ a ∂μ, IsZeroOrProbabilityMeasure (κ a)) (h1 : m1 ≤ m) (h2 : m2 ≤ m)
    (hp1 : IsPiSystem p1) (hp2 : IsPiSystem p2) (hpm1 : m1 = generateFrom p1)
    (hpm2 : m2 = generateFrom p2) (hyp : IndepSets p1 p2 κ μ) :
    Indep m1 m2 κ μ := by
  intro t1 t2 ht1 ht2
  ...
  | basic t ht =>
    refine IndepSets.indep_aux hκ h2 hp2 hpm2 hyp ht (h1 _ ?_) ht2
    rw [hpm1]
    exact measurableSet_generateFrom ht
  | compl t ht iht =>
    filter_upwards [iht, hκ] with a ha hκa
    letI := hκa
    rcases eq_zero_or_isProbabilityMeasure (κ a) with hk | hk
    · simp [hk]
    · letI := hk
      ...
      rw [this, Set.inter_comm t t2,
        ...
        mul_comm (1 - κ a t), ENNReal.mul_sub (fun _ _ ↦ measure_ne_top (κ a) _), mul_one,
        mul_comm]
...
theorem IndepSets.indep {m1 m2 m : MeasurableSpace Ω} {κ : Kernel α Ω} {μ : Measure α}
    [IsZeroOrMarkovKernel κ] {p1 p2 : Set (Set Ω)} (h1 : m1 ≤ m) (h2 : m2 ≤ m)
    (hp1 : IsPiSystem p1) (hp2 : IsPiSystem p2) (hpm1 : m1 = generateFrom p1)
    (hpm2 : m2 = generateFrom p2) (hyp : IndepSets p1 p2 κ μ) :
    Indep m1 m2 κ μ :=
  IndepSets.indep_of_ae_isZeroOrProbabilityMeasure
    (Filter.Eventually.of_forall fun _ ↦ inferInstance) h1 h2 hp1 hp2 hpm1 hpm2 hyp

theorem IndepSets.indep'_of_ae_isZeroOrProbabilityMeasure {_mΩ : MeasurableSpace Ω}
    {κ : Kernel α Ω} {μ : Measure α} {p1 p2 : Set (Set Ω)}
    (hκ : ∀ᵐ a ∂μ, IsZeroOrProbabilityMeasure (κ a))
  ...
  IndepSets.indep_of_ae_isZeroOrProbabilityMeasure hκ
    (generateFrom_le hp1m) (generateFrom_le hp2m) hp1 hp2 rfl rfl hyp
...
theorem IndepSets.indep' {_mΩ : MeasurableSpace Ω}
    ...
    (hp1 : IsPiSystem p1) (hp2 : IsPiSystem p2) (hyp : IndepSets p1 p2 κ μ) :
    Indep (generateFrom p1) (generateFrom p2) κ μ :=
  IndepSets.indep'_of_ae_isZeroOrProbabilityMeasure
    (Filter.Eventually.of_forall fun _ ↦ inferInstance) hp1m hp2m hp1 hp2 hyp
...
theorem iIndepSet.indep_generateFrom_of_disjoint {s : ι → Set Ω}
  ...
  classical
  have hκ : ∀ᵐ a ∂μ, IsZeroOrProbabilityMeasure (κ a) := by
    refine hs.ae_isProbabilityMeasure.mono ?_
    intro a ha
    letI := ha
    infer_instance
  rw [← generateFrom_piiUnionInter_singleton_left, ← generateFrom_piiUnionInter_singleton_left]
  refine
    IndepSets.indep'_of_ae_isZeroOrProbabilityMeasure hκ
...
theorem indep_iSup_of_disjoint {m : ι → MeasurableSpace Ω}
  ...
  classical
  have hκ : ∀ᵐ a ∂μ, IsZeroOrProbabilityMeasure (κ a) := by
    refine h_indep.ae_isProbabilityMeasure.mono ?_
    intro a ha
    letI := ha
    infer_instance
  refine
    IndepSets.indep_of_ae_isZeroOrProbabilityMeasure hκ
      (iSup₂_le fun i _ => h_le i) (iSup₂_le fun i _ => h_le i) ?_ ?_
...
theorem iIndepSets.iIndep (m : ι → MeasurableSpace Ω)
  ...
  classical
  have hκ : ∀ᵐ a ∂μ, IsZeroOrProbabilityMeasure (κ a) := by
    refine h_ind.ae_isProbabilityMeasure.mono ?_
    intro a ha
    letI := ha
    infer_instance
  intro s f
  ...
  · intro _
    filter_upwards [h_ind.ae_isProbabilityMeasure] with a ha
    letI := ha
    simp
  · intro a S ha_notin_S h_rec hf_m
    ...
    have h_indep : Indep m_p (m a) κ μ := by
      ...
      exact IndepSets.indep_of_ae_isZeroOrProbabilityMeasure hκ hm_p (h_le a) hp (h_pi a)
        hS_eq_generate (h_generate a) (iIndepSets.piiUnionInter_of_notMem h_ind ha_notin_S)
