...
/-- If a `NullMeasurable` function is ae equal to `0` or `1`, then its integral is equal to the
measure of the set where it equals `1`. -/
lemma integral_of_ae_eq_zero_or_one (hXmeas : NullMeasurable X μ)
    (hX : ∀ᵐ ω ∂μ, X ω = 0 ∨ X ω = 1) :
    μ[X] = μ.real {ω | X ω = 1} := by
  let hXmeas' : AEMeasurable X μ := hXmeas.aemeasurable
...
/-- If a null measurable random variable is ae equal to `0` or `1`, then one minus its expectation
is equal to the probability that it equals `0`. -/
lemma integral_one_sub_of_ae_eq_zero_or_one (hXmeas : NullMeasurable X μ)
    (hX : ∀ᵐ ω ∂μ, X ω = 0 ∨ X ω = 1) : ∫ ω, 1 - X ω ∂μ = μ.real {ω | X ω = 0} := by
  calc
    _ = μ.real {ω | 1 - X ω = 1} :=
      integral_of_ae_eq_zero_or_one
        ((aemeasurable_const (b := 1).sub hXmeas.aemeasurable).nullMeasurable)
...
/-- If a null measurable random variable is ae equal to `0` or `1`, then its variance is the
product of
...
lemma variance_of_ae_eq_zero_or_one {μ : Measure Ω} [IsZeroOrProbabilityMeasure μ]
    (hXmeas : NullMeasurable X μ) (hX : ∀ᵐ ω ∂μ, X ω = 0 ∨ X ω = 1) :
  ...
  simpa [Pi.mul_def, integral_of_ae_eq_zero_or_one, integral_one_sub_of_ae_eq_zero_or_one, mul_comm,
    condVar_bot hXmeas', *] using condVar_of_ae_eq_zero_or_one bot_le hXmeas' hX
