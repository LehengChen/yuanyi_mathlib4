...
lemma iIndepFun_of_covariance_strongDual [NormedSpace ℝ E]
  ...
  classical
  refine iIndepFun.iIndepFun_process₀ (fun t s ↦ hX.aemeasurable ⟨t, s⟩) fun I J ↦
...
lemma iIndepFun_of_covariance_inner [InnerProductSpace ℝ E]
  ...
  hX.iIndepFun_of_covariance_strongDual fun t₁ t₂ ht s₁ s₂ L₁ L₂ ↦ by
...
lemma iIndepFun_of_covariance_eq_zero {X : (t : T) → (s : S t) → Ω → ℝ}
    ...
    ProbabilityTheory.iIndepFun (fun t ω s ↦ X t s ω) P :=
  hX.iIndepFun_of_covariance_inner fun _ _ h' _ _ _ _ ↦ by
...
lemma indepFun_of_covariance_strongDual [NormedSpace ℝ E]
  ...
  refine IndepFun.process_indepFun_process₀
    (fun s ↦ hXY.aemeasurable (.inl s)) (fun t ↦ hXY.aemeasurable (.inr t)) fun I J ↦
...
lemma indepFun_of_covariance_inner [InnerProductSpace ℝ E]
  ...
  hXY.indepFun_of_covariance_strongDual fun s t L₁ L₂ ↦ by
...
lemma indepFun_of_covariance_eq_zero {X : S → Ω → ℝ} {Y : T → Ω → ℝ}
    (hXY : IsGaussianProcess (Sum.elim X Y) P) (h : ∀ s t, cov[X s, Y t; P] = 0) :
    IndepFun (fun ω s ↦ X s ω) (fun ω t ↦ Y t ω) P :=
  hXY.indepFun_of_covariance_inner fun _ _ _ _ ↦ by
