...
omit [IsSFiniteKernel κ] [IsSFiniteKernel η] in
theorem Kernel.integral_fn_integral_add ⦃f g : β × γ → E⦄ (F : E → E')
    (hf : ∀ᵐ x ∂κ a, Integrable (fun y => f (x, y)) (η (a, x)))
    (hg : ∀ᵐ x ∂κ a, Integrable (fun y => g (x, y)) (η (a, x))) :
    ∫ x, F (∫ y, f (x, y) + g (x, y) ∂η (a, x)) ∂κ a =
      ∫ x, F (∫ y, f (x, y) ∂η (a, x) + ∫ y, g (x, y) ∂η (a, x)) ∂κ a := by
  refine integral_congr_ae ?_
  filter_upwards [hf, hg] with _ h2f h2g
...
theorem Kernel.integral_integral_add ⦃f g : β × γ → E⦄ (hf : Integrable f ((κ ⊗ₖ η) a))
  ...
  (Kernel.integral_fn_integral_add id hf.ae_of_compProd hg.ae_of_compProd).trans <|
...
theorem Kernel.integral_integral_sub ⦃f g : β × γ → E⦄ (hf : Integrable f ((κ ⊗ₖ η) a))
    ...
    ∫ x, ∫ y, f (x, y) - g (x, y) ∂η (a, x) ∂κ a =
      ∫ x, ∫ y, f (x, y) ∂η (a, x) ∂κ a - ∫ x, ∫ y, g (x, y) ∂η (a, x) ∂κ a :=
  (Kernel.integral_fn_integral_sub id hf.ae_of_compProd hg.ae_of_compProd).trans <|
...
theorem Kernel.continuous_integral_integral :
  ...
  simp_rw [← lintegral_fn_integral_sub (‖·‖ₑ) (L1.integrable_coeFn _).ae_of_compProd
    (L1.integrable_coeFn g).ae_of_compProd]
...
theorem integral_fn_integral_add_comp ⦃f g : γ → E⦄ (F : E → E')
    (hf : ∀ᵐ x ∂κ a, Integrable f (η x)) (hg : ∀ᵐ x ∂κ a, Integrable g (η x)) :
...
theorem integral_integral_add_comp ⦃f g : γ → E⦄ (hf : Integrable f ((η ∘ₖ κ) a))
  ...
  (integral_fn_integral_add_comp id hf.ae_of_comp hg.ae_of_comp).trans <|
    integral_add hf.integral_comp hg.integral_comp
...
theorem integral_integral_sub_comp ⦃f g : γ → E⦄ (hf : Integrable f ((η ∘ₖ κ) a))
    (hg : Integrable g ((η ∘ₖ κ) a)) :
    ∫ x, ∫ y, f y - g y ∂η x ∂κ a = ∫ x, ∫ y, f y ∂η x ∂κ a - ∫ x, ∫ y, g y ∂η x ∂κ a :=
  (integral_fn_integral_sub_comp id hf.ae_of_comp hg.ae_of_comp).trans <|
    integral_sub hf.integral_comp hg.integral_comp
...
theorem continuous_integral_integral_comp :
  ...
  simp_rw [← lintegral_fn_integral_sub_comp (‖·‖ₑ) (L1.integrable_coeFn _).ae_of_comp
    (L1.integrable_coeFn g).ae_of_comp]
