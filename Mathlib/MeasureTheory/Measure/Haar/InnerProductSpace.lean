...
theorem OrthonormalBasis.measurePreserving_measurableEquiv (b : OrthonormalBasis ι ℝ F) :
  ...
  have hmap :
      map (⇑b.repr.symm) (EuclideanSpace.basisFun ι ℝ).toBasis.addHaar = b.toBasis.addHaar := by
    simpa using
      (Basis.map_addHaar (EuclideanSpace.basisFun ι ℝ).toBasis
        b.repr.symm.toContinuousLinearEquiv)
  simpa [OrthonormalBasis.measurableEquiv] using b.addHaar_eq_volume.symm.trans hmap.symm
