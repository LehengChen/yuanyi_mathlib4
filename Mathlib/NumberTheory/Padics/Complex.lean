...
theorem norm_eq_norm' : (‖·‖ : ℂ_[p] → ℝ) = Valued.v.norm := by
  ...
  · intro x
    ...
    calc
      ...
      _ = ↑(Valued.v (x : ℂ_[p])) := by rw [valuation_extends]
      _ = ↑(embedding ((PadicComplex.valued p).v.restrict (x : ℂ_[p]))) := by
        exact congrArg (fun y : ℝ≥0 => (y : ℝ))
          (embedding_restrict (PadicComplex.valued p).v (x : ℂ_[p])).symm
